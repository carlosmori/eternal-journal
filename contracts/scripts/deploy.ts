...
async function main() {
  ...
  const OWNER_ADDRESS = process.env.OWNER_ADDRESS || (await ethers.provider.getSigner()).address;

  console.log('Deploying EternalJournalPureOnChainV2 (UUPS proxy)...');
  console.log('Admin / Pauser / Upgrader:', OWNER_ADDRESS);

  const EternalJournalV2 = await ethers.getContractFactory('EternalJournalPureOnChainV2');
  ...
  console.log('');
  console.log('Update ETERNAL_JOURNAL_ADDRESS in apps/web/src/lib/contract.ts with:');
