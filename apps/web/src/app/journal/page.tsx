...
export default function JournalPage() {
  ...
  const [shareConfirm, setShareConfirm] = useState<{ entryIndex: string | number; entry: JournalEntry } | null>(null);
  ...
  const handleShareToCommunity = useCallback(
    (entryIndex: number | string, entry: JournalEntry) => {
  ...
  const handleConfirmShare = useCallback(async () => {
    ...
    const sourceEntryId = idStr.startsWith('web2-') ? idStr.slice(5) : idStr.startsWith('web3-') ? idStr.slice(5) : undefined;
    try {
                  ...
                  >
                    ...
                    {jwt && web2User ? (
                      <div className="px-3 py-2.5 mb-1">
                        <p className="text-sm font-medium text-violet-900 dark:text-white truncate">{web2User.name}</p>
                        <p className="text-xs text-violet-500 dark:text-violet-400 truncate">{web2User.email}</p>
                      </div>
                    ) : (
                      <button
                        onClick={() => { setMenuOpen(false); setSignInModalOpen(true); }}
                    ...
                    {jwt && (
                      <>
                        <div className="h-px bg-violet-200/40 dark:bg-violet-700/30 my-1" />
                        <button
                          onClick={() => { logoutWeb2(); setMenuOpen(false); }}
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-violet-700 dark:text-violet-300 leading-relaxed">
                Share this entry anonymously with the community? It will be reviewed before publishing.
              </p>
